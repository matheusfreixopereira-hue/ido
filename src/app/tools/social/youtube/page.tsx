...
export default function YouTubePage() {
  ...
  const cW = CANVAS_W;
  const cH = activeTool === "banner" ? CANVAS_H_BANNER : activeTool === "thumbnail" ? Math.round(CANVAS_W * 9 / 16) : CANVAS_W;
  ...
  const exportImage = useCallback(() => {
    ...
    out.width = tool.w; out.height = tool.h;
    ...
    ctx.drawImage(src, 0, 0, tool.w, tool.h);
    out.toBlob(blob => {
      if (!blob) return;
          ...
          <div style={{ display: "grid", gridTemplateColumns: "400px 1fr", gap: 32, alignItems: "start" }}>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}>
                      <canvas ref={canvasRef}
                        style={{ borderRadius: activeTool === "profile" ? "50%" : 8, cursor: dragging ? "grabbing" : "grab", boxShadow: "0 4px 20px rgba(0,0,0,0.4)", maxWidth: "100%" }}
            ...
            <div>
              ...
              <div style={{ background: "#fff", borderRadius: 20, overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.4)" }}>
                {/* YT top bar */}
                <div style={{ background: "#fff", padding: "8px 16px", borderBottom: "1px solid #e5e5e5", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ color: "#ff0000", fontWeight: 900, fontSize: 18 }}>▶</span>
                    <span style={{ color: "#000", fontWeight: 900, fontSize: 16 }}>YouTube</span>
                  </div>
                  <div style={{ background: "#f0f0f0", borderRadius: 20, padding: "4px 16px", fontSize: 12, color: "#555" }}>Search</div>
                </div>
                ...
                <div style={{ width: "100%", background: "#1a1a1a", overflow: "hidden", position: "relative" }}>
                  {imgSrc && activeTool === "banner" ? (
                    ...
                    <div style={{ height: 100, background: "linear-gradient(135deg, #1a1a1a, #2d2d2d)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 12 }}>Channel banner</span>
                    </div>
                  ) : (
                    <div style={{ height: 100, background: "#2d2d2d", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 12 }}>Upload banner to preview</span>
                    </div>
                  )}
                </div>
                ...
                <div style={{ padding: "16px 20px", borderBottom: "1px solid #e5e5e5" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    {/* Profile photo */}
                    <div style={{ width: 64, height: 64, borderRadius: "50%", overflow: "hidden", flexShrink: 0, background: "#e0e0e0", border: "3px solid white", marginTop: -20 }}>
                      {imgSrc && activeTool === "profile" ? (
                        <YTProfilePreview canvasRef={canvasRef} size={64} />
                      ) : (
                        <div style={{ width: 64, height: 64, background: "#ff0000", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 900, fontSize: 24 }}>
                          {channelName.charAt(0)}
                        </div>
                      )}
                    </div>
                    <div>
                      <p style={{ fontWeight: 700, color: "#000", fontSize: 18, marginBottom: 2 }}>{channelName}</p>
                      <p style={{ color: "#606060", fontSize: 13 }}>@{channelName.toLowerCase().replace(/\s+/g, "")} · {subscribers}</p>
                      <p style={{ color: "#606060", fontSize: 12, marginTop: 4 }}>Your channel description goes here...</p>
                    </div>
                    <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                      <button style={{ padding: "8px 16px", background: "#000", color: "#fff", border: "none", borderRadius: 20, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Subscribe</button>
                    </div>
                  </div>

                  {/* Nav tabs */}
                  <div style={{ display: "flex", gap: 24, marginTop: 16, borderBottom: "1px solid #e5e5e5", paddingBottom: 0 }}>
                    {["Home", "Videos", "Shorts", "Playlists", "Posts"].map((tab, i) => (
                      <span key={tab} style={{ fontSize: 13, fontWeight: i === 0 ? 700 : 400, color: i === 0 ? "#000" : "#606060", paddingBottom: 10, borderBottom: i === 0 ? "2px solid #000" : "none", cursor: "pointer" }}>{tab}</span>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ padding: 16 }}>
                  <p style={{ fontWeight: 700, color: "#000", marginBottom: 12, fontSize: 14 }}>For you</p>
                    ...
                    {Array.from({ length: 3 }).map((_, i) => (
                      <div key={i}>
                        {imgSrc && activeTool === "thumbnail" && i === 0 ? (
                          ...
                          <div style={{ aspectRatio: "16/9", background: `hsl(${i * 60}, 20%, 85%)`, borderRadius: 8 }} />
                        )}
                        <p style={{ fontSize: 12, color: "#000", fontWeight: 600, marginTop: 6, lineHeight: 1.3 }}>Video title #{i + 1}</p>
                        <p style={{ fontSize: 11, color: "#606060" }}>{channelName}</p>
                      </div>
                    ))}
                  </div>
                </div>
...
function YTBannerPreview({ canvasRef }: { canvasRef: React.RefObject<HTMLCanvasElement | null> }) {
  ...
  useEffect(() => {
    ...
    if (!src || !dst) return;
    dst.width = 600; dst.height = 100;
    const ctx = dst.getContext("2d")!;
    ctx.drawImage(src, 0, 0, 600, 100);
  });
  return <canvas ref={ref} style={{ width: "100%", height: 100, display: "block", objectFit: "cover" }} />;
}
